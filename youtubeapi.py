...
def clean_text(text):
    ...
    return clean_text
def count_sentiments(comments, analyzer):
...
def retrieve_comments(video_id, api_key, page_token=None , max_comments =10):
    ...
    if response.status_code ==200:
        ...
        return None, None
def analyze_all_comments(video_id, api_key):
    ...
    while next_page_token is not None:
        ...
        if comments:
            totals = count_sentiments(comments, analyzer)
            sentiment_totals['positive'] += totals['positive']
            sentiment_totals['negative'] += totals['negative']
            sentiment_totals['neutral'] += totals['neutral']

    return sentiment_totals


def print_comments_and_count_sentiments(comments, totals):
...
if video_id:
    try:
        ...
        sentiment_totals = print_comments_and_count_sentiments(comments, sentiment_totals)
        ...
        while next_page_token is not None:
            ...
            if see_more.lower() == 'see more' :
                ...
                sentiment_totals= print_comments_and_count_sentiments(sentiment_totals)
                
                print_comments_and_count_sentiments(more_comments)

            else:
                sentiment_totals = analyze_all_comments(video_id, api_key)
